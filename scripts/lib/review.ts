...
import { loadBank, loadGames, TEMPLATES } from "./configs";
import { REPO_ROOT } from "./env";

/**
 * Generates the §7.2 approval surface: every bank entry grouped by bank,
 * phone-readable. ☐ = pending, ☑ = approved. Written to out/review/ (brief
 * location) and docs/reports/ (committed, renders on GitHub).
 */
export function generateReviewCopy(): { approved: number; total: number } {
  const lines: string[] = [
