...
- **2026-08-12 · No gameplay footage** (Frank): SIREN videos use production-grade branded abstract motion (`scripts/gen-brand-broll.ts`, tagged SIREN_BRAND_BROLL, unwatermarked/publishable) plus real refx.gg storefront captures (`scripts/capture-site.ts`, our own property) instead of gameplay capture. §7.4's placeholder gate is unchanged — watermarked dev placeholders remain unpublishable. CAPTURE-KIT gameplay session becomes an optional future upgrade.
